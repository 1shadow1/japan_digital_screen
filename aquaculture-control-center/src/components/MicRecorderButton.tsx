...
const MicRecorderButton: React.FC<Props> = ({ onPartial, onFinal, onDialog }) => {
  ...
  const interruptPlaybackAndNotifyStop = (reason?: string) => {
    try {
      ...
      try { ttsPlayerRef.current?.reset() } catch {}
      }
    ...
    ws.onmessage = (evt) => {
      ...
      if (typeof data === 'string') {
        try {
          ...
          if (msg.type === "dialog" && msg.stream === false) {
            try { ttsPlayerRef.current?.playCollected() } catch (e) {}
          }
          if (msg && msg.type === "audio" && msg.mime === "audio/mpeg" && typeof msg.data_b64 === "string") {
            if (ttsPlayerRef.current && audioElRef.current) {
              ttsPlayerRef.current.appendBase64(msg.data_b64 as string);
            }
            return;
          }
          // 建立/过滤会话 ID（如果后端提供）
